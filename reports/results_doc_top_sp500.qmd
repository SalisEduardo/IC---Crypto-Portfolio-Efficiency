---
title: "Top S&P500 Rolling Efficiency"
format: 
  html:
    self-contained: true
execute: 
...
deltaH_melt_360_top <- read.csv("stocks_rolling_deltaH/Top_components/deltaH_melt_360.csv") |>
...
deltaH_melt_720_top <- read.csv("stocks_rolling_deltaH/Top_components/deltaH_melt_720.csv") |>
...
deltaH_melt_1080_top <- read.csv("stocks_rolling_deltaH/Top_components/deltaH_melt_1080.csv")  |>
...
status_efficiency_stocks_360_top <- summarise_status_efficiency(deltaH_melt_360_top)
status_efficiency_stocks_720_top <- summarise_status_efficiency(deltaH_melt_720_top)
status_efficiency_stocks_1080_top <- summarise_status_efficiency(deltaH_melt_1080_top)
...
for(t in unique(deltaH_melt_360_top$Ticker)){
  cat(paste("###",t,"\n"))
  
  print(plot_time_varying_efficiency(deltaH_melt_360_top,t,'360'))
  cat("\n")
...
for(t in unique(deltaH_melt_720_top$Ticker)){
  cat(paste("###",t,"\n"))
  
  print(plot_time_varying_efficiency(deltaH_melt_720_top,t,'720'))
  cat("\n")
...
for(t in unique(deltaH_melt_1080_top$Ticker)){
  cat(paste("###",t,"\n"))
  
  print(plot_time_varying_efficiency(deltaH_melt_1080_top,t,'1080'))
  cat("\n")
...
status_efficiency_stocks_360_top |>
  kable(format = "html") |>
...
status_efficiency_stocks_720_top |>
  kable(format = "html") |>
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
...
status_efficiency_stocks_1080_top |>
  kable(format = "html") |>
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
...
for(q in quantiles_ineff){
  ...
  result <- consolidate_backtests(deltaH_melt_360_top, threshold_column=q, position=position,tab_returns= returns_long_top20 )[[2]]|> 
...
for(q in quantiles_ineff){
  ...
  result <- consolidate_backtests(deltaH_melt_720_top, threshold_column=q, position=position,tab_returns= returns_long_top20 )[[2]]|> 
    dplyr::arrange(desc(`Annualized Return`)) |>
    dplyr::mutate(Significance_level = q) |>
...
for(q in quantiles_ineff){
  ...
  result <- consolidate_backtests(deltaH_melt_1080_top, threshold_column=q, position=position,tab_returns= returns_long_top20 )[[2]] |> 
    dplyr::arrange(desc(`Annualized Return`)) |>
    dplyr::mutate(Significance_level = q) |>
...
table_results |> 
  bind_rows() |> 
  writexl::write_xlsx("backtests_trading/kpis_top_sp500.xlsx")
