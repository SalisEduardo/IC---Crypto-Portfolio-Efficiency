...
backtest_strategy <- function(df, ticker, threshold_column='q70',position='Short-Inefficiency',tab_returns=NULL) {
  
  if(is.null(tab_returns)){
    ...
    tab_returns <- crypto_returns_long
  }
  ...
  ticker_df <- df|> 
    dplyr::filter(Ticker == ticker) |>
    dplyr::left_join(tab_returns,by=c('date','Ticker')) 
